...
function _498_Business_Error($msg = "")
{
	headerAndDie("HTTP/1.1 498 Business Error : " . $msg);
}
...
function raiseHttpStatus(HttpStatusException $ex)
{
	switch ($ex->getCode()) {
		case 400:
			_400_Bad_Request($ex->getMessage());
			break;
		case 404:
			_404_Not_Found($ex->getMessage());
			break;
		case 498:
			_498_Business_Error($ex->getMessage());
		case 499:
			_499_Authentication_Error($ex->getMessage());
			break;
		case 500: // Ici on veut savoir ce qu'il se passe dans le serveur
			...
			break;
		default:
			throw new Exception("Http Status Exception not managed : " . $ex->getCode());
	}
}
