...
class LoginPostController extends AbstractController implements IController
{
	...
	{
		function logNoticeColored(string $message): void
		{
			$cyan = "\033[36m";
			$reset = "\033[0m";
			$time = date('Y-m-d H:i:s');
			$formatted = "{$cyan}[{$time}] [NOTICE] {$message}{$reset}\n";
		...
		logNoticeColored("Checking form inputs, referer: " . ($_SERVER['HTTP_REFERER'] ?? 'unknown'));
