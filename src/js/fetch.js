...
const defaultResponseCallback = function (response) {
  ...
  if (!response.ok) {
    console.log(
      "Status Text : " + response.statusText + " || code : " + response.status
    );
...
export function myFetch(
  ...
  fetch(url, {
    ...
    .then((response) =>
      responseCallback == null
        ? defaultResponseCallback(response)
        : responseCallback(response)
    )
    .then((data) =>
      dataCallback == null ? defaultDataCallback(data) : dataCallback(data)
    )
    .catch((error) =>
      errorCallback == null ? defaultErrorCallback(error) : errorCallback(error)
    );
}
